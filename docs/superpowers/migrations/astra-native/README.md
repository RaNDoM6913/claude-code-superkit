...
Baseline commit: `e0314ad46442729578bfd43f409e52b6843282fc`. The machine-readable ledger contains 544 tracked and untracked repository surfaces. Current baseline evidence is `pass-with-environment-resolution`; see `baseline.json` and `evidence/W00A/baseline/` for exact exits and outputs.
...
| W00B | Behavioral corpus, scoring, and model baseline | in-progress | [W00B-1 fixtures](evidence/W00B/lookup-fixtures.md); [W00B-2 command gate](evidence/W00B/failed-verification.md); [W00B-3 scope/evidence](evidence/W00B/scope-evidence.md); [W00B-4 output/execution](evidence/W00B/output-execution.md) |
...
- W00B-1 adds executable clean/defect lookup fixtures and deterministic red/green evidence. W00B-2 adds the partial command-verification scorer; W00B-3 adds strict scope/evidence boolean gates; W00B-4 adds output/execution gates and per-gate diagnostics. Remaining scenarios, scoring dimensions, runner, and all model baseline/acceptance runs are pending. Next bounded block: verification-evidence artifact hashes (see W00B-4 checkpoint). The original baseline.json remains immutable.
