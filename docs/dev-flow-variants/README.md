...
Four hand-authored SVG visualizations of the `/dev` 16-phase orchestrator. The diagrams render the 15 working phases as nodes; Phase 0 (`read-docs` — load project context) is the entry preamble folded into the `/dev` core. Each one shows the 4 parallel reviewers that run during the Review phase (Review ×4) in a way that fits its composition.
...
Linear left-to-right sequence. A gradient track carries 15 numbered working-phase nodes through three colored phase zones (Planning / Execution / Quality). At node 12 the track fans out into 4 reviewer pills below. The `/dev <task>` pill floats at top center instead of bracketing the timeline with input/output pills.
...
## 02 · Radial clock
All 15 working phases orbit a central `/dev` core on a compact circle. Three colored arcs (indigo / amber / emerald) mark the phase boundaries around the rim. The Review step is a larger filled node with 4 small satellite reviewers clustered beside it.
