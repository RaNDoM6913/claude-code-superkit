...
- **Fix (statusline `ctx` bar never rendered on Claude Code 2.1.x):** `getContextBudget` read the `context_window` stdin field as a bare token count, but since 2.1.x the CLI sends it as an **object** (`{ total_input_tokens, context_window_size, used_percentage, current_usage, … }`). An object is non-null, so the native branch was entered, failed its `typeof used !== 'number'` guard and returned `''` — and because that return sat *above* the transcript fallback, the fallback became dead code and the bar vanished entirely for every subscriber on a current CLI. The object shape is now parsed properly: `used_percentage` first (the same number `/context` prints, so the two can never disagree), else `total_input_tokens ÷ context_window_size`; a `context_window` object carrying no usable numbers falls through to the transcript scan as intended. The legacy numeric field and the `CLAUDE_CONTEXT_TOKENS_MAX` precedence chain are untouched. Verified against the payload schema extracted from the 2.1.206 binary, across 8 branches: real 2.1.x payload, object without `used_percentage`, session start (0 tokens → `0%`), empty object → transcript fallback, legacy numeric, no `context_window`, `rate_limits` co-render, and no-stdin (exit 0, no bar).
- **Statusline (active-task segment removed):** the `task` segment — which printed `currentTask` from `.claude/.task-state.json` — is gone from `helpers/statusline.cjs` (and from the repo's own `statusline-local.cjs`). Nothing ever wrote that file automatically: `pre-compact-save.sh` and `session-context-restore.sh` only *read* it, and `packages/core/CLAUDE.md` asks Claude to hand-write it for unusually long sessions. In practice the segment rendered in exactly one project (this repo, carrying a stale hand-typed string) and was blank everywhere else, so it bought a stale-data hazard and no signal. `.task-state.json` itself is untouched and still feeds compaction continuity via those two hooks — only its statusline readout is dropped. The header comment was corrected in the same pass: it advertised `profile, stacks, migration count, hooks`, none of which the line has rendered since 1.5.2.
- **Statusline (final layout + now synced to consumers):** the shipped `helpers/statusline.cjs` line was trimmed to the essentials — `v<version> | branch | model ⚙effort ⟁ULTRA | ctx bar | 5h | W` — dropping the `[S]`/stack/component-count clutter (only a `[strict]` badge surfaces the non-default profile). AND it is no longer installer-only: `superkit-update.sh` now syncs `statusline.cjs` under the SAME pristine rule as everything else — a consumer who forked it keeps their fork (preserved + reported), an untouched copy gets the upstream improvements, an absent one is created (34-case updater suite: absent→created + forked→preserved).
