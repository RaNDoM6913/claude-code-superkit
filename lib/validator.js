...
function checkGitignoreHooks(projectDir) {
  ...
  for (const line of lines) {
    ...
    if (trimmed === '.claude/scripts/' || trimmed === '.claude/scripts' || trimmed === '.claude/scripts/*') {
      warn('  ❌ .gitignore blocks hook scripts: "' + trimmed + '"');
      warn('     Remove this line — hooks MUST be tracked in git.');
      warn('     Without this, other users get NO enforcement.');
      return false;
    }
  }
  return true;
...
export function validate(claudeDir, projectDir) {
  ...
  if (!checkGitignoreHooks(projectDir)) {
    ok = false;
  }
