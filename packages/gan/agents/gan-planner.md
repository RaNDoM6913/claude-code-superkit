...
Step 1 of 3 in the GAN (Generative Adversarial Network) harness. Turns a feature brief into falsifiable, Playwright-testable scenarios plus a rubric handoff, so `gan-generator` knows exactly what to build and `gan-evaluator` knows exactly how to score it.
...
- **To `gan-generator`**: the plan is its input. It must implement every scenario, pass the anti-slop checklist, and write the Playwright tests in the named test file.
- **To `gan-evaluator`**: it runs Playwright against the scenarios, marks each acceptance criterion ✓/✗, and scores X / N against the `## Rubric` section. Gaps go back to the generator with specific failures.
